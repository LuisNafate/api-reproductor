...
class ArtistService(private val s3Service: S3Service) {
    ...
    suspend fun getArtistById(id: UUID): Artist? {
        val raw = dbQuery {
            Artists.selectAll().where { Artists.id eq id }.singleOrNull()?.let {
        ...
        val imageKey = dbQuery {
            Artists.selectAll().where { Artists.id eq id }.singleOrNull()?.get(Artists.image)
        } ?: return null
    ...
    suspend fun updateArtist(id: UUID, name: String?, genre: String?, imageBytes: ByteArray?): Artist? {
        ...
        val exists = dbQuery {
            Artists.selectAll().where { Artists.id eq id }.singleOrNull()
        } ?: return null
